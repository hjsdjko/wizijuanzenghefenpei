...
, "usersId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
            ...
            //级联表 机构
            //级联表
            JigouEntity jigou = jigouService.selectById(forum.getJigouId());
            if(jigou != null){
    ...
    */
    ...
    public R save(@RequestBody ForumEntity forum, HttpServletRequest request){
        ...
        if(false)
            ...
            forum.setJigouId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
...
//                            forumEntity.setJigouId(Integer.valueOf(data.get(0)));   //机构 要改的
